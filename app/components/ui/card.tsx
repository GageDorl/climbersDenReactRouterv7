...
const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ className = '', ...props }, ref) => (
    <div
      ref={ref}
      className={`rounded-lg border border-default bg-surface text-primary shadow-sm ${className}`}
      {...props}
    />
  )
);
...
const CardHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className = '', ...props }, ref) => (
    <div
      ref={ref}
      className={`flex flex-col text-primary space-y-1.5 p-2 px-4 ${className}`}
      {...props}
    />
  )
);
